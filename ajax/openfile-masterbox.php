...
if(!empty($_POST["session_id"]) AND !empty($_POST["sku"])){ // Принимаем данные
	...
	if ($file_exists) {
		...
		$ean_mbox = $array['Barcode'];
		$master_box_height = $array['Height'];
		$master_box_length = $array['Length'];
		$master_box_width = $array['Width'];
		$master_box_weight = $array['Weight'];
